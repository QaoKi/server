...
int EasyClient::SendCmd(const char* chCmd)
{
	...
	if (strCmd.compare("login") == 0){
		CELLSendStream logout;
		//写入要发送的命令
		logout.setNetMsgCmd(CMD_LOGOUT);
		char name[32] = "zhaofei";
		logout.WriteArray(name,sizeof(name));
		char pawss[32] = "123456";
		logout.WriteArray(pawss, sizeof(pawss));
		SendData((netmsg_DataHeader* )logout.getData());
	}
