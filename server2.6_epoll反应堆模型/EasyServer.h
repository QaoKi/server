...
/*
	...
	2.2：加入运行日志记录
			把之前的xPrintf宏定义移除，想看方法的话可以看之前的版本

	2.3：如果创建多个EasyServer对象，其中一个关闭的时候，会直接把网络环境清理了，其他的就不能用了
		 申请一个新类CELLNetWork，处理网络的初始化和清理

	2.4：之前服务器和客户端的数据都是用struct，改为通过字节流传输
		 CELLStream是基类，CELLMsgStream是处理消息类
...
private:
	...
	//接受客户端连接
	int Accept(int sock,int events);
