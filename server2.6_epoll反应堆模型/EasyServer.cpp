...
{
	CELLEpollEvent myevent(1);
		
	myevent.eventAdd(_sock, this);
	EventCallBack cb = std::bind(&EasyServer::Accept, this, placeholders::_1, placeholders::_2);
	myevent.eventSet(_sock, EPOLLIN, cb);

		
	while (pThread->isRun())
	{			
		Time4Msg();
		int ret = myevent.eventStart();
		if (ret < 0)
		{
			CELLLog::Info("server epoll exit\n");
			pThread->Exit();
			break;
		}
		else if (ret == 0)
...
int EasyServer::Accept(int sock, int events)
{
