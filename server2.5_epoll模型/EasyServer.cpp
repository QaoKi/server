...
bool EasyServer::Run(CELLThread* pThread)
{
	...
	ep_in.events = EPOLLIN;
	ep_in.data.fd = _sock;
	if (epoll_ctl(epfd, EPOLL_CTL_ADD, _sock, &ep_in) < 0)
	{
	...
	{			
		Time4Msg();
		int nReady = epoll_wait(epfd, &ep_out, 1, 0);
		if (nReady < 0)
		{
		...
		//如果 _sock 没有就绪，或者不是读事件
		else if (nReady == 0 || !ep_out.events & EPOLLIN) {
			CELLThread::Sleep(10);
			continue;
		}

		if (ep_out.data.fd == _sock)
		{
			//接受新连接
			Accept();
		}
