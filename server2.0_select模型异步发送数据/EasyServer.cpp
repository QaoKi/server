...
int EasyServer::Bind(const char* ip, unsigned short port)
{
	...
	if (SOCKET_ERROR == ret) {
#ifdef _WIN32
		printf("error,bind port<%d> fail...error code:%d\n", port, GetLastError());
#else
		perror("bind port fail:\n");
#endif
	}
	else {
		printf("bind port<%d> succ...\n", port);
	}
	return 0;
}
...
int EasyServer::Listen(int n)
{
	...
	{
#ifdef _WIN32
		printf("socket=<%d> error,listen port fail...error code:%d\n", _sock,GetLastError());
#else
		perror("listen port fail:\n");
#endif
	}
	else {
		printf("socket=<%d>listen port succ...\n",_sock);
	}
	return ret;
...
SOCKET EasyServer::Accept()
{
	...
	if (cSock <= 0) {
#ifdef _WIN32
		printf("socket=<%d>error,accept invalid client SOCKET...error code:%d\n", _sock, GetLastError());
#else
